...
class Margin3DASRDeicticAug(Margin3DASRDeictic, Base3DAug):
    def __init__(self, workspace, heightmap_size, device, lr=1e-4, gamma=0.9, sl=False, num_primitives=1,
        ...
        Margin3DASRDeictic.__init__(self, workspace, heightmap_size, device, lr, gamma, sl, num_primitives, patch_size, num_rz, rz_range, margin, margin_l, margin_weight, softmax_beta)

    def _loadBatchToDevice(self, batch):
        return Base3DAug._loadBatchToDevice(self, batch)

    def update(self, batch):
        loss, td_error = Margin3DASRDeictic.update(self, batch)
