...
class Margin3DASRDrQ(DQN3DASRDrQ, MarginBase):
    def __init__(self, workspace, heightmap_size, device, lr=1e-4, gamma=0.9, sl=False, num_primitives=1,
                 patch_size=24, num_rz=8, rz_range=(0, 7 * np.pi / 8), margin='l', margin_l=0.1, margin_weight=0.1,
                 softmax_beta=100):
        DQN3DASRDrQ.__init__(self, workspace, heightmap_size, device, lr, gamma, sl, num_primitives, patch_size, num_rz, rz_range)
    ...
    def calcMarginLoss(self):
        ...
        action = self.loss_calc_dict['action']
        action_idx_dense = action[:, 0] * self.heightmap_size + action[:, 1]
        q1_margin_loss = self.getMarginLossSingle(q1_output.reshape(q1_output.size(0), -1), action_idx_dense, is_experts.repeat(self.M), True)
        q2_margin_loss = self.getMarginLossSingle(q2_output, action[:, 2], is_experts.repeat(self.M))
